...
{
    public class InventarioVehiculos1Controller : ApiController
    {
        private ModelDB db = new ModelDB();

        // GET: api/InventarioVehiculos
        public IQueryable<ITEM_FACTURABLE> GetITEM_FACTURABLE()
        {
            return db.ITEM_FACTURABLE;
        }

        // GET: api/InventarioVehiculos/5
        [ResponseType(typeof(ITEM_FACTURABLE))]
        public IHttpActionResult GetInventarioVehiculos(string estado)
        {
            var VEHICULO = db.ITEM_FACTURABLE.Where(it=> it.ESTADO_NEGOCIO.VALOR == estado).Select(v=>v.VEHICULO).ToList();
            if (VEHICULO == null)
            {
                return NotFound();
            }
        ...
        // DELETE: api/InventarioVehiculos/5
        [ResponseType(typeof(ITEM_FACTURABLE))]
